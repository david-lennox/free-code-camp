            ...
            <div style={{clear: "both"}} id={self.props.recipe.name} className='recipe'>
                ...
                {self.props.currentRecipe  ? (
                        <div className='recipeDetails'>
                            <ul>{self.props.ingredients}</ul>
                            <p>{self.props.recipe.description}</p>
                        </div>
                ) : null}             
            </div>
        )
...
var RecipeApp = React.createClass({
    getRecipes: function(){
        var firstRecipes;
        try {
            firstRecipes = JSON.parse(localStorage.getItem("_dave004_recipes"));
            if(!firstRecipes[0].name.length > 0) throw(new Error("something is not working."));
        } catch (error) {
            console.log(error);
    ...
    render: function(){
        ...
        if(this.state.recipes != null){
            recipes = this.state.recipes.map(function(recipe, index){
                ...
                else ingredients = <p className="warning">Please add some ingredients!</p>
        ...
        else recipes = "No recipes in local storage";
